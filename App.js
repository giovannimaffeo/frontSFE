...
const DATA = [
  {
    ...
    foto_usuario:
      'https://www.bu.edu/disability/files/2019/02/no_profile_photo.jpg',
  },
];
...
const CustomDrawer = props => {
  ...
  const texto_inicial = (
    <Text style={styles.texto_inicial}>Clique para colocar seu nome!</Text>
  );
  const texto_username = (
    <Text style={[styles.texto_username, {color: colorsList.dark_terciaria}]}>
      {username}
    </Text>
  );
    ...
    <View>
      <View
        ...
        <TextInput
          ...
          onChangeText={text => {
            text != null ? set_numero(20 - text.length) : set_contador(20);
            set_username(text);
          }}
          value={username}
          maxLength={20}
        ...
        <Text
          style={{color: 'rgba(0, 0, 0, 0.25)', fontSize: screenWidth * 0.033}}>
          {numero}
        </Text>
      </View>
      ...
      <View style={{alignItems: 'flex-end'}}>
        <TouchableOpacity
          onPress={() => {
            set_esta_mudando(false);
            armazena_username(username);
            armazena_numero(numero);
          }}
          style={{
            alignItems: 'flex-end',
  ...
  const texto_nao_mudando = (
    <View style={{height: screenWidth * 0.3, width: screenWidth * 0.58}}>
      <TouchableOpacity
        style={[styles.botao_username, {marginTop: screenWidth * 0.08}]}
        onPress={() => set_esta_mudando(true)}>
  ...
  async function reset_dados() {
    ...
    var foto_perfil_armazenada = await AsyncStorage.getItem(
      '@AppSFE:foto_perfil',
    );
    ...
    username_armazenado
      ? set_numero(20 - username_armazenado.length)
      : set_numero(20);
    setToken(password);
  }
  ...
  function showToken() {
    Alert.alert(
      'Número de Inscrição da Semana',
      username
        ? `Eaí ${username}, o seu número é:  ${token}`
        : `O seu número é:  ${token}`,
      [{text: 'Beleza!'}],
    );
  }
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.menu_lateral_container}>
        <View
          style={[
            styles.perfil_container,
            {
              backgroundColor: colorsList.quaternaria,
              borderBottomColor: colorsList.terciaria,
            },
          ]}>
          <View
            style={{
        ...
        <View style={{backgroundColor: colorsList.primaria, flex: 1}}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Linking.openURL('https://forms.gle/aZtsrLLHxRDHv6wx5')
            }>
            <View
              style={{
                ...Platform.select({
            ...
            <View>
              <Text style={[styles.texto, {color: colorsList.terciaria}]}>
                Processo Seletivo
              </Text>
            </View>
          </TouchableOpacity>
...
export default function App() {
  const isSignedIn = async () => {
    ...
    return token !== null
      ? (setSigned(true), setSignLoaded(true))
      : (setSigned(false), setSignLoaded(true));
  };
